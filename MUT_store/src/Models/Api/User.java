...
public static Response loginAPI(Usuario user) {
    try {
        ...
        if (tokenFile.exists()) {
            // Se o token já existir, exclui o arquivo antigo para garantir que o novo token será salvo
            if (!tokenFile.delete()) {
                System.err.println("Falha ao excluir o token antigo.");
            }
        }
    ...
    public static boolean sendDevRequest() {
        try {
            ...
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(API_URL + "/users/give/dev"))
                    .header("Content-Type", "application/json; utf-8")
                    .header("Authorization", "Bearer " + token)
                    .POST(HttpRequest.BodyPublishers.ofString("{}")) // Corpo vazio como na API original
                    .build();

            // Envia a requisição e recebe a resposta
            HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());

            // Verifica se a resposta tem código 200
            if (response.statusCode() == 200) {
                // Parseia o JSON da resposta
                ObjectMapper objectMapper = new ObjectMapper();
                JsonNode jsonResponse = objectMapper.readTree(response.body());

                // Verifica se o código da resposta da API é 0
                if (jsonResponse.get("code").asInt() == 0) {
                    // Pega a mensagem e exibe
                    String message = jsonResponse.get("message").asText();
                    System.out.println("Sucesso: " + message);
                    return true;
                } else {
                    // Se o code não for 0, exibe a mensagem de erro
                ...
                System.err.println("Falha na requisição. Código de resposta: " + response.statusCode());
                return false;
            }
        } catch (Exception e) {
            e.printStackTrace();
